...
namespace PSOOffsets {
    ...
    const uintptr_t NAME_PRIVATE_OFFSET = 0x18;
    const uintptr_t CLASS_PRIVATE_OFFSET = 0x10;
    const uintptr_t OUTER_PRIVATE_OFFSET = 0x20;
    const uintptr_t OBJECT_FLAGS_OFFSET = 0x08;
    const uintptr_t INTERNAL_INDEX_OFFSET = 0x0C;
}
...
public:
    ...
    ~ProSoccerDumper() {
        if (logFile.is_open()) {
            ...
            std::string msg = "Dump Basariyla Tamamlandi!\nDosya: " + logPath + 
                              "\nNesne Sayisi: " + std::to_string(totalObjects) + 
                              "\nIsim Sayisi: " + std::to_string(totalNames);
    ...
    void FindPSOOffsets() {
        ...
        unsigned char gobjects_pattern1[] = { 0x48, 0x8B, 0x05, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8B, 0x0C, 0xC8, 0x48, 0x8D, 0x04, 0xD1 };
        const char* gobjects_mask1 = "xxx????xxxxxxx";
        
        unsigned char gobjects_pattern2[] = { 0x48, 0x8D, 0x0D, 0x00, 0x00, 0x00, 0x00, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8B, 0x45 };
        ...
        if (addr) {
            ...
            gObjectsFound = true;
            logFile << "[+] GObjects Bulundu (Pattern 1): 0x" << std::hex << PSOOffsets::GObjects << std::dec << "\n";
        } else {
            ...
            if (addr) {
                ...
                PSOOffsets::GObjects = (addr + relOffset + 7) - baseAddress;
                gObjectsFound = true;
                logFile << "[+] GObjects Bulundu (Pattern 2): 0x" << std::hex << PSOOffsets::GObjects << std::dec << "\n";
            }
        }
        ...
        unsigned char gnames_pattern[] = { 0x48, 0x8D, 0x35, 0x00, 0x00, 0x00, 0x00, 0xEB, 0x00, 0x48, 0x8D, 0x0D };
        ...
        if (addr) {
            ...
            gNamesFound = true;
            logFile << "[+] GNames Bulundu: 0x" << std::hex << PSOOffsets::GNames << std::dec << "\n";
        }
        logFile << "\n";
    }
    ...
    void FindGObjectsManual() {
        for (uintptr_t i = baseAddress; i < baseAddress + moduleSize - 0x30; i += 8) {
            __try {
                ...
                if (maxElements > 1000 && maxElements < 1000000 && numElements > 0 && numElements <= maxElements) {
                    ...
                    if (objectsPtr > baseAddress && objectsPtr < baseAddress + moduleSize) {
                        ...
                        gObjectsFound = true;
                        logFile << "[+] GObjects Manuel Bulundu: 0x" << std::hex << PSOOffsets::GObjects << std::dec << "\n";
                        return;
                    }
                }
            }
        ...
        for (int blockIdx = 0; blockIdx < 8192 && nameCount < 100000; blockIdx++) {
            ...
            for (int i = 0; i < 65536; i++) {
                ...
                __try {
                    ...
                    if (IsValidPointer(entryAddr)) {
                        compIndex = *(uint32_t*)entryAddr;
                        if (compIndex > 0 && compIndex < 10000000) {
                            char* name = (char*)(entryAddr + 4);
                            if (IsValidPointer((uintptr_t)name) && name[0] != '\0' && isprint((unsigned char)name[0])) {
                                size_t len = strlen(name);
                                if (len > 0 && len < 256) {
                                    strcpy_s(tempBuffer, name);
                                    valid = true;
                                }
                            }
                        }
                ...
                if (valid) {
                    nameMap[compIndex] = std::string(tempBuffer);
                    nameCount++;
                }
            }
        }
    ...
    void DumpGObjects() {
        ...
        for (uint32_t i = 0; i < numElements && objCount < 100000; i++) {
            __try {
                ...
                logFile << "[" << objCount << "] Addr: 0x" << std::hex << objectPtr << std::dec 
                        << " | NameIdx: " << namePrivate.ComparisonIndex 
                        << " | InternalIdx: " << internalIndex << " | ";
    ...
    // C2712 Hata Çözümü: SEH kullanan fonksiyonda yerel nesne (vector vb.) olamaz.
    // Bu yüzden Heap taraması tamamen ayrıştırılmış wrapper üzerinden yürütülür.
    bool SafeCheckPotentialUObject(uintptr_t address) {
        __try {
    ...
    void ScanHeapForObjects(std::vector<uintptr_t>& objects, int& objCount) {
        ...
        while (VirtualQuery((LPCVOID)address, &mbi, sizeof(mbi)) && objCount < 50000) {
            if (mbi.State == MEM_COMMIT && (mbi.Protect & (PAGE_READWRITE | PAGE_EXECUTE_READWRITE))) {
                ...
                for (uintptr_t scan = (uintptr_t)mbi.BaseAddress; scan < regionEnd - 0x30; scan += 8) {
                    if (SafeCheckPotentialUObject(scan)) {
    ...
    void DumpSingleObject(uintptr_t objPtr, int index) {
        __try {
            ...
            logFile << "[" << index << "] 0x" << std::hex << objPtr << std::dec
                    << " | NameIdx:" << namePrivate.ComparisonIndex << " | Flags:" << flags;
    ...
    void DumpObjectsAlternative() {
        ...
        totalObjects = (int)potentialObjects.size();
    }
    ...
    void DumpImportantClasses() {
        ...
        const char* importantClasses[] = {
            ...
            "PlayerCameraManager", "HUD", "Level", "Player", "Ball"
        };
...
// ============================================
// DLL Ana Giriş ve Thread İşlemleri
// ============================================
DWORD WINAPI DumperThread(LPVOID lpParam) {
    ...
    std::cout << "\n[*] Dump tamamlandi! Dosya masaustune kaydedildi.\nCikmak icin Enter tuşuna basin...\n";
    std::cin.get();
